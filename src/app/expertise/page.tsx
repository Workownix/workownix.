...
export default function Expertise() {
  ...
  const headingText = "OUR EXPERTISE ";
    ...
    <main className="bg-black text-white overflow-hidden">
      ...
      <section className="relative py-16 overflow-hidden">
        ...
        <motion.h2
          ...
          whileInView="visible"
        >
          {"We are Experts in  ".split("").map((l, i) => (
        ...
        <div className="flex justify-center items-center gap-4 mb-6 px-4">
          ...
          >
            <ChevronLeft className="w-5 h-5 sm:w-6 sm:h-6 text-blue-400" />
          </motion.button>
          ...
          >
            {isPlaying ? (
              <Pause className="w-5 h-5 sm:w-6 sm:h-6 text-green-400" />
            ) : (
        ...
        <div className="bg-gradient-to-r from-red-600 to-red-700 text-white py-2 px-4 mb-4 mx-4 sm:mx-8 rounded-lg">
          <div className="flex items-center gap-3">
            <div className="bg-white text-red-600 px-2 py-1 rounded text-xs sm:text-sm font-bold">
              LIVE
            </div>
            <div className="text-sm sm:text-base font-semibold">
              🔥 Our Premium Services - Transforming Businesses Worldwide
            </div>
          </div>
        </div>
          ...
          >
            {[...services, ...services, ...services].map((service, idx) => (
              <motion.div
                ...
                className="w-72 bg-gray-800 rounded-lg overflow-hidden flex-none hover:bg-gray-700 transition-all duration-300 shadow-lg hover:shadow-2xl border border-gray-700 hover:border-gray-600"
              >
        ...
        <div className="flex justify-center mt-6 px-4">
          <div className="bg-black/50 backdrop-blur-sm rounded-full px-4 py-2 border border-white/10">
            <span className="text-xs sm:text-sm text-gray-400 mr-3">Speed:</span>
            <button 
              onClick={() => document.documentElement.style.setProperty('--marquee-speed', '60s')}
              className="text-xs sm:text-sm px-2 py-1 rounded text-gray-300 hover:text-white transition-colors"
            >
              Slow
            </button>
            <button 
              onClick={() => document.documentElement.style.setProperty('--marquee-speed', '30s')}
              className="text-xs sm:text-sm px-2 py-1 rounded text-blue-400 hover:text-blue-300 transition-colors mx-2"
            >
              Normal
            </button>
            <button 
